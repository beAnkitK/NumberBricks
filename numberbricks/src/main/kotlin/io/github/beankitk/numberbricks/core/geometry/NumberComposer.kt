...
/**
 * Central stateful coordinator for NumberBricks composable that manages number state
 * and digit geometry.
 *
 * A [NumberComposer] manages the transformation of a number into per-digit geometry.
 * It parses the number into digits, maintains per-position digit state, and
 * provides access to brick model for each digit generated via [DigitBuilder].
 *
 * The composer operates on fixed [digitGridSpec] and [geometryProps], which define
 * the grid constraints and shared geometry configuration used to construct digits.
 *
 * It maintains both [currentNumber] and [previousNumber] to support transition-aware
 * rendering. Digit-level transitions are tracked using [DigitSlot].
 *
 * Before use, [initiate] must be called to construct the builder and prepare internal
 ...
 *
 * **Note:** Currently, this composer operates only on non-negative integer values.
 * Any negative input is converted to its absolute value before processing.
 *
...
interface NumberComposer<T : Brick<T>> {
     ...
     *
     * This value is initialized with the provided initial number (converted to its
     * absolute value) and becomes readable only after [initiate] completes.
     *
     * During updates, the new number is fully processed (digit parsing, slot updates,
     * and geometry composition) before this value is updated, ensuring consistent state
     * for transition-aware consumers.
     *
     ...
     *
     * This remains `null` after [initiate]. It is assigned when [updateNumber] is called
     * with a value different from the current number, capturing the value before the change.
     */
     ...
     *
     * This constructs the [digitBuilder], prepares default bricks, and processes
     * the initial number into digit slots while caching required brick models. This
     * must be called before accessing any other API.
     *
     ...
     *
     * The input is converted to its absolute value. If the value has not changed,
     * no work is performed. Otherwise, digits are re-parsed, slots are updated,
     * and required brick data is generated and cached.
     *
     * The new number is first processed by parsing its digits and computing the
     * corresponding brick model before updating [currentNumber] and [previousNumber].
     * This ensures a consistent transition state during updates.
     *
     ...
     *
     * The index is zero-based and represents the digit place in reverse order
     * (least significant first): It can be interpreted as a power of ten:
     * - `0` -> 10^0 = 1 -> ones place
     * - `1` -> 10^1 = 10 -> tens place
     * - `2` -> 10^2 = 100 -> hundreds place
     * - `3` -> 10^3 = 1000 -> thousands place and so on
     *
     * Each [DigitSlot] encapsulates the state for that position, including the
     * current digit and its previous value, enabling transition-aware rendering.
     *
     * The number of valid indices is determined by [getDigitCount]. Accessing an
     * index outside this range returns `null`.
     *
     ...
     *
     * The composer caches brick model per digit on demand, the first time a digit appears
     * in the number during the current lifecycle.
     *
     * If this method is called after [updateNumber] and the digit is part of the
     * current number, a valid brick model is guaranteed to be returned.
     *
     * Returns `null` only if the digit has not yet been encountered in the current
     * lifecycle of the composer.
     *
     ...
     *
     * Used as placeholder representation when brick model for a specific digit is
     * not available. The default bricks are computed once on first access and cached
     * for reuse. Subsequent calls return the cached result to avoid redundant
     * construction.
     *
     ...
     * If the composer is not initialized, this call is a no-op.
     *
     * After disposal, [initiate] must be called again before use, otherwise it would
 ...
 *
 * Manages number state, digit transitions, geometry composition using [DigitBuilder]
 * and brick caching. It maintains a per-position [DigitSlot] structure to track
 * transitions between digits and caches brick model per digit for efficient reuse.
 *
 * Internally:
...
class DefaultNumberComposer<T : Brick<T>>(
    ...
    // Stores the current number internally. It should not be accessed until the composer has been initialized.
    ...
    /**
     * Parses a number into individual digits in reverse order. Returns digits from
     * least to most significant (ones place first). Example: 456 -> [6, 5, 4]
     */
