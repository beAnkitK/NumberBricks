 ...
 *
 * This [SizeProvider] computes block sizes using per-column widths and
 * per-row heights, enabling non-uniform sizing across the grid. Each block’s
 * size is derived from its [Position], making this provider dependent on
 * [PositionProvider].
 *
 * **Requirements:**
 ...
 *
 * Input arrays define relative proportions and are normalized during attachment
 * so that column widths and row heights sum to total column and row count
 * respectively. These arrays can be further modified during geometry composition
 * and are published as [io.github.beankitk.numberbricks.core.geometry.Meta] for
...
open class VariableSize(
     ...
     *
     * This method is called for each block during geometry composition to allow
     * adjustment of its final size. The provided [baseSize] is the computed size;
     ...
     *
     * This method is called once per digit before geometry composition to allow
     * adjustment of column widths. The provided [colWidths] are normalized; the
     ...
     *
     * If overriding, ensure the array is normalized, or use [normalizeColWidths]
     * and cache the result to avoid repeated normalization.
     *
     * @param digit The digit being composed
     ...
     *
     * This method is called once per digit before geometry composition to allow
     * adjustment of row heights. The provided [rowHeights] are normalized; the
     * returned array is used directly.
     *
     * The returned array must:
     * - Match the row count
     * - Contain only non-negative values
     *
     * If overriding, ensure the array is normalized, or use [normalizeRowHeights]
     * and cache the result to avoid repeated normalization.
     *
     * @param digit The digit being composed
    ...
    /**
     * Scales row height weights proportionally so that their total equals the number of rows.
     * Each value then represents its proportional share in grid units (where 1f = one row).
     *
    ...
    /**
     * Meta group for [VariableSize], defining all keys published during
     * provider execution.
     *
     * These keys allow other providers to access computed column widths
     * and row heights within the same [ProviderScope].
     */
