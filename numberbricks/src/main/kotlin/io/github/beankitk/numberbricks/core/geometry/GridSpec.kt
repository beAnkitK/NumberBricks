 ...
 *
 * Specifies the logical grid in which a digit geometry is composed. The grid is defined
 * by its row and column count, while [brickCount] determines how many cells are
 * occupied within that grid.
 *
 * The number of bricks may be less than or equal to the total number of cells
 * (`rows * cols`), allowing sparse layouts where some grid positions are empty.
 *
 * Preconditions:
 * - `rows > 0`
 * - `cols > 0`
 ...
 * @param brickCount Number of bricks in the grid. Must be greater than 0 and
 *                   less than or equal to `rows * cols`
