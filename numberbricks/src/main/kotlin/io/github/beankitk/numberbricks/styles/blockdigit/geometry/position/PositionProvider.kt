 ...
 *
 * A [PositionProvider] is a [GeometryProvider] responsible for defining the
 * grid position (row and column) of each block for a given digit. These values
 * establish the structural geometry of the digit and are used during block assembly
 * to construct the final brick model.
 *
 * Implementations must provide positions aligned with the digit grid, where each
 * block maps to a valid (row, column) within the configured [GridSpec].
 *
...
/**
 * Base implementation of [PositionProvider] for manually defining per-block
 * position data for each digit.
 *
 * This allows specifying [Position] for all blocks per digit using [DigitData],
 * giving full control over block placement in grid during geometry composition.
 * Subclasses define provider data as a list of [Position] for each digit,
 * aligned with the provider's [grid constraints][providerGridSpec].
 *
 * @param providerGridSpec The [GridSpec] defining the grid constraints this
 *        provider is bound to and must align its position data with
 */
