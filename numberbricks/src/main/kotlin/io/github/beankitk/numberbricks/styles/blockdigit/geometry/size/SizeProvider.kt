 ...
 *
 * A [SizeProvider] produces a [Size] for every block in the current digit. The returned sizes are
 * used during geometry composition to determine each block's width and height.
 *
 * Size values must be expressed in grid-relative units, where `1f` represents the width or height
 * of a single grid cell. Values greater than `1f` span multiple cells, while values less than `1f`
 * occupy a fraction of a cell.
 *
