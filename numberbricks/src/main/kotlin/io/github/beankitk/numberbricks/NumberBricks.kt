...
import io.github.beankitk.numberbricks.core.NumberBricksImpl
import io.github.beankitk.numberbricks.utils.defaultAnimationSpec

/**
 * Displays a single numeric digit (0–9) using 13 rectangular bricks arranged in a 3×5 grid. Each
 * brick forms part of the digit’s shape, and transition between digits are smoothly animated
 ...
 * @param brickSize optional uniform size for each bricks of the numberbrick. The composable
 *   measures itself as `width = 3 * brickSize` and `height = 5 * brickSize` maintaining aspect
 ...
 * @param animateDigits if true, transitions between different digits are animated. Default to
 *   `false`
 * @param animationSpec the animation spec controlling easing and duration . Supports all compose
 *   animation spec
 * @param animateOnFirstVisible if true, the first appearance of the composable animates appearing
 *   as growing from center to target digit. Has no effect if [animateDigits] is `false`
 ...
 * @param digit the digit to display. Must be in the range 0..9
 * @param modifier modifier applied to the root drawing surface
 * @param brickWidth optional width for each bricks of the numberbrick. The composable measures
 *   itself as `width = 3 * brickWidth`. If null, the `brickWidth` fallback to default width else it
 *   scale to fit incoming constraints changed with [Modifier] or parent constraints if final width
 *   is larger than parent constraints.
 * @param brickHeight optional height for each bricks of the numberbrick. The composable measures
 *   itself as `height = 5 * brickHeight`. If null, the `brickHeight` fallback to default height
 *   else it scale to fit incoming constraints changed with [Modifier] or parent constraints if
 *   final height is larger than parent constraints.
 * @param digitStyle styling information for the digit bricks. See [DigitStyle] for details
 * @param animateDigits if true, transitions between different digits are animated. Default to
 *   `false`
 * @param animationSpec the animation spec controlling easing and duration . Supports all compose
