 ...
 *
 * A concrete [BaseDigitBuilder] implementation for block-style digits. It performs geometry
 * composition by coordinating four core [GeometryProvider]s that provide the required data to
 * construct each [Block] for a given digit:
 *
 ...
 *
 * The collected geometry from these providers is assembled into the final brick model representing
 * the digit.
 *
...
class BlockDigitBuilder(
    ...
    override fun assembleDefaultBricks(): List<Block> {
        return List(digitGridSpec.brickCount) { index ->
            Block(
                index = index,
