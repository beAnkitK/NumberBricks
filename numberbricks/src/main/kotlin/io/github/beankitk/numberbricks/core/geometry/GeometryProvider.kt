 ...
 *
 * A [GeometryProvider] produces a specific aspect of digit geometry (for example, `position`,
 * `size`, `offset`) for each brick. For a given digit, it returns a list of elements of type [T],
 * where each element corresponds to exactly one brick in the current [ProviderScope].
 *
 * Providers participate in the geometry composition pipeline coordinated by [DigitBuilder]. Each
 * provider contributes one dimension of data that is later combined to form the final brick model.
 *
 * A provider is identified by a unique [ProviderKey], which is used to declare dependencies via
 * [dependsOn] and to retrieve results from other providers.
 *
 * Providers declare compatibility with grid constraints using [matchesWith]. After acceptance, they
 * are initialized with [attachWith], which supplies the resolved grid constraints and shared
 * [GeometryProps].
 *
 * Providers execute within a [ProviderScope], where they can access the current digit and results
 * of other providers. Dependencies are resolved by the [DigitBuilder] and executed before this
 * provider. Implementations must ensure that [provideData] returns exactly
 * `providerGridSpec.brickCount` elements.
 *
 * **Important:** Declare each provider [key] in a `companion object` so it can be referenced
 * without creating an instance. The property name must be `key` for consistent referencing and
...
interface GeometryProvider<T> {
    ...
    /**
     * Identifies this provider and the type of data it produces. Used to declare dependencies and
     * retrieve results across providers.
     */
     ...
     *
     * For adaptive providers, this is derived during [attachWith]. For fixed providers, this is
     * predefined and must match the incoming grid constraints.
     */
     ...
     *
     * Invoked during builder construction before initialization. Implementations should verify that
     * the provider can operate with the given [digitGridSpec]. Returning [Consent.Reject] excludes
     * this provider from execution.
     *
     ...
     *
     * Executed once during builder construction after compatibility is accepted. Implementations
     * may cache values or prepare internal state required during execution.
     *
     ...
     *
     * Returns a list of values aligned with bricks, where each element corresponds to a single
     * brick for the current digit. The returned list size must be equal to
     * `providerGridSpec.brickCount`.
     *
     * @receiver ProviderScope that provides the execution environment required to compute this
     *   provider's data
     */
 ...
 *
 * Adaptive providers accept any grid constraints by default and derive their [providerGridSpec]
 * during [attachWith]. This allows them to operate across different grid configurations.
 *
 * Subclasses implement [onAttachWith] to perform initialization using the resolved grid constraints
 * and geometry properties. The [providerGridSpec] is available after attachment.
 *
...
abstract class AdaptiveProvider<T> : GeometryProvider<T> {
    ...
    final override val providerGridSpec
        get() =
            _providerGridSpec ?: error("providerGridSpec accessed before attachWith() was called.")

    /**
     * Always accepts the provided grid constraints. Override to enforce additional compatibility
     * checks.
     */
     ...
     *
     * Called once during builder construction after [matchesWith] succeeds. Use this to cache
     * computed values or prepare internal state required during execution.
     *
     * @param digitGridSpec Provides the resolved grid constraints
     * @param geometryProps Provides the shared geometry configuration
     */
 ...
 *
 * Fixed providers operate on predefined grid constraints and only accept configurations that
 * exactly match [providerGridSpec]. This makes them suitable for layouts with strict structural
 * requirements.
 *
...
abstract class FixedProvider<T> : GeometryProvider<T> {
    ...
    override fun matchesWith(digitGridSpec: GridSpec): Consent {
        val matches =
            providerGridSpec.rows == digitGridSpec.rows &&
                providerGridSpec.cols == digitGridSpec.cols &&
                providerGridSpec.brickCount == digitGridSpec.brickCount

        if (!matches) {
            return Consent.Reject(
                "Provider requires gridSpec ${providerGridSpec.asString()} but got ${digitGridSpec.asString()}"
            )
        }
 ...
 *
 * Prefer using this function to construct provider results. Alternative approaches (such as manual
 * iteration or mapping) may introduce additional allocations (for example, iterators), while this
 * uses direct indexed construction.
 *
